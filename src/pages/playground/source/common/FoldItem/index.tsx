...
export const FoldItem: React.FC<IFormItemProps & { expand?: boolean }> & {
  Base?: React.FC;
  Extra?: React.FC;
} = observer(
  ({ className, style, children, expand: outerExpand, ...props }) => {
    const prefix = usePrefix('fold-item');
    const field = useField();
    if (outerExpand) {
      ExpandedMap.set(field.address.toString(), true);
    }
